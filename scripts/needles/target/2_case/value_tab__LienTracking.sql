...
exec AddBreadcrumbsToTable 'sma_TRN_Lienors'
go
---
...
insert into value_tab_Lien_Helper
	...
	where
		code in (select code from conversion.value_lienTracking)
		or
		V.value_id in (select value_id from value_tab_Liencheckbox_Helper)
...
/* ------------------------------------------------------------------------------
Create Lien Types
if a [value] record has a [code] that does not exist in value_code, dissTypeName is set to [value].[code]
...
insert into [sma_TRN_Lienors]
	...
	select
		...
		ISNULL(V.total_value, 0)		 as [lnrnUnCnfrmdLienAmount],
		ISNULL(V.due, 0)				 as [lnrnCnfrmdLienAmount],
