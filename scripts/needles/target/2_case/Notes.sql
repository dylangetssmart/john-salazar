...
insert into [sma_MST_NoteTypes]
	...
	from [JohnSalazar_Needles].[dbo].[case_notes_Indexed]
	union all
	-- from [value_notes].[topic]
	select distinct
		vn.topic,
		vn.topic
	from [JohnSalazar_Needles]..value_notes vn
	except
	select
		nttsDscrptn,
		nttsNoteText
...
insert into [sma_TRN_Notes]
	...
	select
		...
		case
			when n.note_date between '1900-01-01' and '2079-06-06' and
				CONVERT(TIME, ISNULL(n.note_time, '00:00:00')) <> CONVERT(TIME, '00:00:00') then CAST(CAST(n.note_date as DATETIME) + CAST(n.note_time as DATETIME) as DATETIME)
			when n.note_date between '1900-01-01' and '2079-06-06' and
				CONVERT(TIME, ISNULL(n.note_time, '00:00:00')) = CONVERT(TIME, '00:00:00') then CAST(CAST(n.note_date as DATETIME) + CAST('00:00:00' as DATETIME) as DATETIME)
			else '1900-01-01'
	...
	--select *
	from [JohnSalazar_Needles].[dbo].[case_notes_Indexed] n
...
insert into [sma_TRN_Notes]
	...
	select
		...
		case
			when n.note_date between '1900-01-01' and '2079-06-06' and
				CONVERT(TIME, ISNULL(n.note_time, '00:00:00')) <> CONVERT(TIME, '00:00:00') then CAST(CAST(n.note_date as DATE) as DATETIME) + CAST(CAST(n.note_time as TIME) as DATETIME)
			else null
		end			  as notddtcreated,
		null		  as [notnmodifyuserid],
		null		  as notddtmodified,
	...
	from [JohnSalazar_Needles].[dbo].[value_notes] n
...
insert into sma_TRN_NoteContacts
	...
	from [JohnSalazar_Needles]..[value_notes] n
	join [JohnSalazar_Needles]..value_Indexed v
		on v.value_id = n.value_num
