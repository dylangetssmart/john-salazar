/*---
description: Insert users from [staff] into [sma_MST_Users] and seed default users (aadmin, conversion)
steps:
    - Ensure default users exist (aadmin, conversion)
    - Insert users from staff that do not yet exist
    - Seed CaseBrowseSettings for all users
    - Assign roles in Account_UsersInRoles
usage_instructions:
    - Run after [sma_MST_IndvContacts] has been populated
dependencies:
    - [sma_MST_IndvContacts]
    - [JohnSalazar_Needles]..staff
    - [sma_MST_CaseBrowseColumns]
notes: >
    Ensures users are linked to individual contacts and staff.
    Prevents duplicate users by checking source_id / usrsLoginID.
---*/
...
---------------------------------------------------
-- Insert [sma_MST_Users] from [staff] if not exists
---------------------------------------------------
insert into [sma_MST_Users]
	(
		[usrnContactID],
...
insert into Account_UsersInRoles
	...
	select distinct
		src.usrnUserID as user_id,
		2			   as role_id -- Attempting to insert role_id 2
	from sma_MST_Users as src
	where
		not exists (
		 -- Check the target table for the exact (user_id, role_id) pair
		 select
			 1
		 from Account_UsersInRoles as target
		 where target.user_id = src.usrnUserID
			 and target.role_id = 2 -- Only exclude if role_id 2 already exists for this user
		);
