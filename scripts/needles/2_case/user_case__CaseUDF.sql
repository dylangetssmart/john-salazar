...
--join Schechter_Needles..cases_Indexed ci
--	on ci.casenum = pv.casenum
left join [JohnSalazar_Needles].[dbo].user_case_name utn
	on utn.casenum = pv.casenum
		and utn.ref_num = nuf.field_num
		and utn.user_name <> 0;
go

SELECT * FROM user_case_data_pivoted

-- add record per case to track alt_case_num
insert into user_case_data_pivoted
	(
		casenum,
		Attribute,
		Value,
	...
	select
		ci.casenum,
		'Alt Case Num'  as Attribute, -- The fixed Attribute name
		ci.alt_case_num as Value,       -- The value from cases_indexed
		null			as user_name,              -- Set to NULL as this is not a user-specific field
		'Alt Case Num'  as field_title, -- Set to match the Attribute
		0				as field_num,                -- Use a placeholder ID if required
		'Text'			as UDFType,              -- Placeholder for data type
		'alpha'			as field_type,           -- Placeholder for field type
		255				as field_len,               -- Placeholder for length
		'cases_indexed' as table_name,  -- Source table reference
		'alt_case_num'  as column_name,  -- Source column reference
		null			as DropDownValues          -- Set to NULL as this is not a drop-down field
	from [JohnSalazar_Needles]..cases_indexed ci
	where
		ci.alt_case_num is not null -- Only insert records that actually have a value
		and ci.alt_case_num <> ''
go

---- add record per case to track alt_case_num ('Bank Loan Number')
--insert into user_case_data_pivoted
--	(
--		casenum,
--		Attribute,
...
--	select
--		ci.casenum,
--		'Bank Loan Number' as Attribute, -- The fixed Attribute name
--		ci.alt_case_num	   as Value,       -- The value from cases_indexed
--		null			   as user_name,              -- Set to NULL as this is not a user-specific field
--		'Bank Loan Number' as field_title, -- Set to match the Attribute
--		0				   as field_num,                -- Use a placeholder ID if required
--		'Text'			   as UDFType,              -- Placeholder for data type
--		'alpha'			   as field_type,           -- Placeholder for field type
--		255				   as field_len,               -- Placeholder for length
...
--SELECT *
--FROM Schechter_Needles.INFORMATION_SCHEMA.COLUMNS
--WHERE TABLE_NAME = 'user_case_data'
--ORDER BY COLUMN_NAME;

--SELECT casenum, Mediation_Date,Mediation_Time, Method_of_Sign_Up, Wanted, Decision_Maker FROM Schechter_Needles..user_case_data where casenum = 240962
--SELECT * FROM user_case_data_pivoted where casenum = 240962 and attribute in ('Mediation_Date', 'Mediation_Time', 'Wanted', 'Method_of_Sign_Up', 'Decision_Maker')
--SELECT * FROM Schechter_Needles..NeedlesUserFields nuf where nuf.table_name='user_case_data' --where nuf.column_name = 'Method_of_Sign_Up'
...
insert into [sma_TRN_UDFValues]
	...
	select
		...
		case
			when pe.field_type = 'name' then CONVERT(VARCHAR(MAX), ioci_name.UNQCID)
	...
	left join IndvOrgContacts_Indexed ioci_name
		on ioci_name.SAGA = pe.user_name
			and pe.field_type = 'name' -- Only attempt to join for name fields
	...
	left join IndvOrgContacts_Indexed ioci_staff
		on ioci_staff.source_id = pe.Value
			...
			and pe.field_type = 'staff' -- Only attempt to join for staff fields
