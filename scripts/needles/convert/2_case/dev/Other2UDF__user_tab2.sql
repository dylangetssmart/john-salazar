...
-- Dynamically get all columns from [JohnSalazar_Needles]..user_tab2_data for unpivoting
...
FROM ( 
    ...
    FROM [JohnSalazar_Needles]..user_tab2_data ud
...
begin
	insert into [sma_MST_UDFDefinition]
		...
		join [JohnSalazar_Needles].[dbo].[user_tab2_matter] M
			on M.mattercode = mix.matcode
				and M.field_type <> 'label'
		join (select distinct fieldTitle from Other2UDF) vd
