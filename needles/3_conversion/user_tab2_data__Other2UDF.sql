...
declare @sourceTable SYSNAME = 'user_tab2_data';		-- source data EAV table
declare @targetTable NVARCHAR(25) = 'Other2'			-- target UDF screen				[sma_MST_UDFDefinition].[udfScreenName]
declare @targetTable_grid NVARCHAR(25) = 'UDFs Grid2'	-- target UDF screen (grid)			[sma_MST_UDFDefinition].[udfScreenName]
...
left join [Needles].[dbo].user_tab2_name utn		-- make sure this is correct: i.e. [user_tab2_name] matches with [user_tab2_data]
...
--select * from user_tab2_data_pivoted
...
declare udf_cursor cursor for select distinct
	...
	join user_tab2_data_pivoted pe
...
-- Final output
--select * from dbo.user_tab2_data_pivoted; -- verify results