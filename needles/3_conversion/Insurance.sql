...
if OBJECT_ID('insurance_contacts_helper', 'U') is not null
begin
...
insert into insurance_contacts_helper
	...
	--select *
	from [Needles].[dbo].[insurance_Indexed] ins
	join [sma_TRN_Cases] cas
		on cas.saga = ins.case_num
...
if OBJECT_ID('multi_party_helper') is not null
begin
	drop table multi_party_helper
end
...
Plaintiff Insurance
*/ ------------------------------------------------------------------------------
exec AddBreadcrumbsToTable 'sma_TRN_InsuranceCoverage'
go
...
insert into [sma_TRN_InsuranceCoverage]
	...
	--select *
	from [Needles].[dbo].[insurance_Indexed] ins
	left join [Needles].[dbo].[user_insurance_data] ud
