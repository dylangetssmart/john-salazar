...
declare @sourceTable SYSNAME = 'user_tab8_data';		-- source data EAV table
declare @targetTable NVARCHAR(25) = 'Other8'			-- target UDF screen				[sma_MST_UDFDefinition].[udfScreenName]
declare @targetTable_grid NVARCHAR(25) = 'UDFs Grid8'	-- target UDF screen (grid)			[sma_MST_UDFDefinition].[udfScreenName]
...
/* ------------------------------------------------------------------------------
Create final staging table [user_tab8_data_pivoted]
- Add context to ##PivotedData such as [user_name] for Contact type UDFs and ROW_ID for Grid Row inserts
*/ ------------------------------------------------------------------------------
if OBJECT_ID('user_tab8_data_pivoted') is not null
	drop table user_tab8_data_pivoted;
...
select
	...
	nuf.column_name,
	nuf.DropDownValues
into user_tab8_data_pivoted
from ##PivotedData pv
join [Needles].[dbo].NeedlesUserFields nuf
	on nuf.column_name = pv.Attribute
		and nuf.table_name = @sourceTable
left join [Needles].[dbo].user_tab8_name utn		-- make sure this is correct: i.e. [user_tab2_name] matches with [user_tab2_data]
...
--select * from user_tab8_data_pivoted
...
insert into [sma_MST_UDFDefinition]
	...
	--select *
	from user_tab8_data_pivoted pe
...
declare udf_cursor cursor for select distinct
	...
	join user_tab8_data_pivoted pe
...
-- Final output
--select * from dbo.user_tab8_data_pivoted; -- verify results